...
impl AuctionRepo {
    pub async fn insert(&self, a: &Auction) -> Result<(), AuctionError> {
        ...
        sqlx::query!(
            "INSERT INTO auctions (id, creator_id, title, description, status, reserve_price,
             server_signature_hex, bb_create_sequence, created_at, updated_at)
             VALUES (?,?,?,?,?,?,?,?,?,?)",
            a.id, a.creator_id, a.title, a.description, status, a.reserve_price,
